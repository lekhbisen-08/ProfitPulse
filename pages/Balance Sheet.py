...
def get_company_data(ticker):
    ...
    return balance_sheet, cash_flow, hist

st.title(f"{st.session_state.tickermain} Financial Data")


ticker = f"{st.session_state.tickermain}.NS"

# Display the data if user enters a ticker
if ticker:
    
    try:
        balance_sheet, cash_flow, hist = get_company_data(ticker)
        
        # Display the financial data
        st.subheader("Balance Sheet")
        st.write(balance_sheet)
        
        st.subheader("Stock Price History (Last 5 Years)")
